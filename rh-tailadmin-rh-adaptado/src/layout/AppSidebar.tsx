...
const AppSidebar: React.FC = () => {
  ...
  const items = filtrarMenuPorRol(navItems, usuario?.rol ?? "rh");
  ...
  useEffect(() => {
    ...
    if (!matched) {
      const direct = items.findIndex((item) => item.path && isActive(item.path));
            ...
            <>
              ...
              >
                ...
                {(isExpanded || isHovered || isMobileOpen) && (
                  <ChevronDownIcon className={`ml-auto h-5 w-5 transition-transform duration-200 ${openSubmenu === index ? "rotate-180 text-brand-500" : ""}`} />
                )}
              </button>
              {(isExpanded || isHovered || isMobileOpen) && (
                <div
        ...
        <Link to="/">
          {(isExpanded || isHovered || isMobileOpen) ? (
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-500 text-lg font-bold text-white">RH</div>
              <div>
                <p className="text-sm font-semibold text-gray-900 dark:text-white">SIE RH</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">TailAdmin adaptado</p>
              </div>
            </div>
          ) : (
      ...
      <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
        <nav className="mb-6">
          <div className="flex flex-col gap-4">
            <div>
              <h2 className={`mb-4 flex text-xs leading-[20px] text-gray-400 uppercase ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>
                {(isExpanded || isHovered || isMobileOpen) ? "Menú principal" : <HorizontaLDots className="size-6" />}
              </h2>
